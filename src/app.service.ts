...
export class AppService {
  ...
  async listCurrencies(): Promise<any> {
    ...
    const lista = await dbo.collection("currencies").find().toArray();

    return lista;

  }

  async getExchangeRate(x: string, y: string, z: Date, w: number): Promise<any> {

    const conn = await MongoClient.connect(url);
    const dbo = await conn.db("bcp");
    var query = { moneda: x, fecha: z };
    var lista = await dbo.collection("exchangerate").find(query).toArray();

    var valor = 1.0 / lista[0].valor;

    query = { moneda: y, fecha: z };
    lista = await dbo.collection("exchangerate").find(query).toArray();
